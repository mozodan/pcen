...
;(function($, undefined) {
	...
	$.widget("vanderlee.coverflow", {
		...
		_create: function() {
			...
			// Mousewheel
      var youMayScroll = true;
			that.element.on('mousewheel', function(event, deltaY, deltaX, deltaF) {
				event.preventDefault();
	        if(deltaY*deltaF > that.options.sensitivity){
            if(youMayScroll){
              if(deltaF > 0){ //Scroll down
              	if(that.options.index == 0){
              		that._setIndex(that.element.find("li").length - 1, true);
              	} else {
	        				that._setIndex(that.options.index - 1, true);
	        			}
              }
              
              if(deltaF < 0){ //Scroll up
                 if(that.options.index == that.element.find("li").length - 1){
