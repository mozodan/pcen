...
Drupal.ajax.prototype.commands.entity_reference_tree_append_children = function(ajax, response, status) {
  $(ajax.wrapper).siblings('.entity-reference-tree-button').removeClass('entity-reference-tree-collapsed');
  ...
  var $tree = $newElement.parents('.entity-reference-tree');
  EntityReferenceTreecheckMaxChoices($tree, false);
}
...
Drupal.behaviors.EntityReferenceTree = {
  attach: function(context, settings) {
    ...
    $('.entity-reference-tree-widget-level-ajax-button.ajax-processed', context).once(function() {
      var $element = $(this).closest('ul.entity-reference-tree-level li');
      var id = $(this).attr('id');
    ...
    $('.entity-reference-tree-button', context).once('entity-reference-tree-button').click(function(e) {
      var $this = $(this);
      if ($this.siblings('ul').length > 0) {
        ...
        $this.siblings('div.entity-reference-tree-widget-level-ajax').find(':input').trigger('mousedown');
      }
    });
    ...
    */
    ...
    $('.entity-reference-tree-level :input', context).filter('[type=radio], [type=checkbox]').once().on('change', function(e) {
      var $this = $(this);
      var $tree = $this.parents('.entity-reference-tree');
      var input_type = $this.is(':checkbox') > 0 ? 'checkbox' : 'radio';

      // Check for max choices.
      EntityReferenceTreecheckMaxChoices($tree, $this);

      // Select parent automatically.
      if ($this.hasClass('select-parents') && $(this).is(':checked')) {
        $this.parents('ul.entity-reference-tree-level li').children('div.form-item').find(':input:not(:checked)').each(function() {
          $(this).attr('checked', 'checked').trigger('change');
        });
      }

      // Cascading selection.
      if ($tree.hasClass('entity-reference-tree-cascading-selection')) {
        var $children = $this.closest('ul.entity-reference-tree-level li').find(':input[id^="' + $this.attr('id') + '-children"]');
        if ($this.is(':checked')) {
      ...
      if ($tree.hasClass('entity-reference-tree-track-list-shown')) {
        var track_list_container = $tree.find('.entity-reference-tree-track-list');

        // Remove the "nothing selected" message if showing - add it later if needed.
        EntityReferenceTreeremoveNothingSelectedMessage(track_list_container);
        if ($this.is(':checked') ) {
          // Control checked - add item to the track list.
          label_element = $this.closest('.form-item').find('label');
          EntityReferenceTreeaddItemToTrackList(
        ...
        EntityReferenceTreeshowNothingSelectedMessage(track_list_container);
      }
    });
      ...
      if ($(this).hasClass('entity-reference-tree-track-list-shown')) {
        ...
        $(track_list_container).click(function(event){
          ...
          if(control_id) {
            ...
            EntityReferenceTreecheckMaxChoices(tree, checkbox);
 ...
 * match the order of the entitys in the checkbox tree.
 *
 ...
 */
function EntityReferenceTreeaddItemToTrackList(track_list_container, item_text, control_id, control_type) {
 ...
 */
function EntityReferenceTreeshowNothingSelectedMessage(track_list_container) {
 ...
 */
function EntityReferenceTreeremoveNothingSelectedMessage(track_list_container) {
  track_list_container.find('.entity_ref_tree_nothing_message').remove();
}
...
function EntityReferenceTreecheckMaxChoices(item, checkbox) {
  var maxChoices = -1;
  try {
